...
// Parse multipart form and extract payload_json + optional screenshot
   ...
   return new Promise((resolve, reject) => {
      form.parse(req, (err, fields, files) => {
         ...
         try {
            ...
            return reject(new Error('Invalid JSON in payload_json'));
         }

         // Dink sends the screenshot (when "Send Image" is enabled) under a
         // field named "file" — png or jpeg, up to 8MB per Dink's own docs.
         const file = Array.isArray(files.file) ? files.file[0] : files.file;

         resolve({ data, file: file || null });
      });
   });
}
...
export default async function handler(req, res) {
   ...
   try {
      if (contentType.includes('application/json')) {
         ...
         const parsed = await parseMultipartPayload(req);
         data = parsed.data;
         file = parsed.file;
      }
   } catch (e) {
   ...
   const embed = {
      title: `<:osrs_rb:1532173160240386108> ${title}`,
